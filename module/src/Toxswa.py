...
class Toxswa(object):
    ...
    def __init__(self, experimentDir, toxswaConfig, generalConfig, outputDir):
        ...
        self.startTime = dt.datetime.strptime(self.generalConfig['startDateSim'],'%d-%b-%Y')
        self.endTime = dt.datetime.strptime(self.generalConfig['endDateSim'],'%d-%b-%Y') + dt.timedelta(hours=23)
        ...
        if not 'MasDwnWatLay' in self.outputVars: self.outputVars.append('MasDwnWatLay')
        if not os.path.exists(self.workDir): os.makedirs(self.workDir)

        # write meteorology file
        tempData = pandas.read_csv(os.path.join(self.inputDir,toxswaConfig['temperatureFile']),
                         header=[0,1],parse_dates=[0],infer_datetime_format=True)
        filter = ((tempData.Time>=self.startTime) & (tempData.Time<=self.endTime)).iloc[:,0]
        tempData = tempData.loc[filter,:]
        fmt = [lambda x: x.strftime("%d/%m/%Y"), lambda x: str(x)]
        util.writeOutputTable(os.path.join(self.workDir,"temperature.met"),tempData, fmt, inclColNames = False, 
                              headerLines = ['* Daily air temperature:', '*'+','.join(tempData.columns.get_level_values(0)) ])
    ...
    def init_reach(self,reach):
        ...
        if reach.skip or (self.txw_exists(reach) and self.hyd_exists(reach) and self.mfs_exists(reach)): return

        hydrologyMassLoadingsTable = pandas.read_csv(os.path.join(self.generalConfig['inputDir'],reach.hydrologyMassLoadingsFile),
                                                     header = [0], parse_dates=[0],infer_datetime_format = True,skiprows = self.HML_skipRows)
        ...
        if firstRow.Time.hour==1:
            hydrologyMassLoadingsTable = pandas.concat([hydrologyMassLoadingsTable.loc[[0,]],hydrologyMassLoadingsTable], ignore_index=True)
    ...
    def write_mfsFile(self,reach,hydrologyMassLoadingsTable):
        massFlowTimestepFcn = lambda row: self.reach_massFlowTimestep(reach,row['DepWat'],row['QBou'])

        massFlowTimestepLength = hydrologyMassLoadingsTable.apply(massFlowTimestepFcn, axis = 1)
        mfsDateTimeColumn = hydrologyMassLoadingsTable.Time.apply(lambda x: x + datetime.timedelta(minutes=30))

        mfsTimestepColumnFcn = lambda x: (x-(mfsDateTimeColumn.iloc[0]-dt.timedelta(hours=1))).total_seconds()/(3600*24)
        mfsTimestepColumn = mfsDateTimeColumn.apply(mfsTimestepColumnFcn)
        mfsTable = pandas.concat([mfsTimestepColumn, mfsDateTimeColumn,massFlowTimestepLength], axis=1)
        mfsTable.columns = ['timeStep','dateTime','timestepLength']

        # Read MFS template file
        with open(os.path.join(self.toxswaConfig['toxswaDir'],self.mfsTemplateFile),'r') as f:
            mfsTempl = f.read()
        
        # Fill in no of upstream reaches and their IDs 
        mfsTempl = re.sub("<upstreamReachIdList>", ', '.join(reach.upstreamIDs), mfsTempl)
        # Formatter to convert table to string to write to MFS file
        datePrintFcn = lambda x: (x - datetime.timedelta(minutes=30)).strftime(Toxswa.dateTimeFormat)
        fmt = [lambda x: "%5.3f" %x,  datePrintFcn,  lambda x: "%.6g" %x]
        mfsStr = mfsTable.to_string(header = False, index = False, formatters = fmt)

        with open(os.path.join(self.workDir, "Reach" + str(reach.ID) + ".mfs"),'w') as mfsFile:
    ...
    def read_mfsfile(self,reachID):
        ...
        mfsTable = pandas.read_csv(mfsFilePath,delim_whitespace=True,
                                   skiprows=7,parse_dates=[1],header=None,date_parser=cu.mfs_dateparser,
                                   names = ['Time','DateTime','MassFluxTimeStep'])
        mfsTable.loc[:,'MassFluxTimeStep'] = pandas.to_timedelta(mfsTable.MassFluxTimeStep,unit='S')
        return mfsTable
    ...
    def reach_massFlowTimestep(self,reach,waterDepth,flowRate):
        ...
        massFlowTimestep = max(self.toxswaConfig.getfloat('minMassFlowTimestep'),min(Toxswa.hydrologyTimeStep,
                                    residenceTime*self.toxswaConfig.getfloat('massFlowTimestepParam')))
        return Toxswa.hydrologyTimeStep/np.floor_divide(Toxswa.hydrologyTimeStep,massFlowTimestep)
    ...
    def write_txwFile(self,reach,hydrologyMassLoadingsTable):
        ...
        varNameWaterBody = ['Len', 'NumSeg', 'WidWatSys', 'SloSidWatSys', 'DepWatDefPer']
        varUnitsWaterBody = ['m', '-', 'm', '-', 'm']
        varWidthWaterBody = [4, 8, 11, 13, 13]

        # Read contents of TXW template file
        txwTemplate_file = open(os.path.join(self.toxswaConfig['toxswaDir'],self.txwTemplateFile),"r")
        ...
        txwContent[lineIdx] = re.sub("<startDateSim>", dt.datetime.strftime(self.startTime,'%d-%b-%Y'), txwContent[lineIdx])
        lineIdx = get_lineIndex(txwContent,"<endDateSim>") - 1
        txwContent[lineIdx] = re.sub("<endDateSim>",  dt.datetime.strftime(self.endTime,'%d-%b-%Y'), txwContent[lineIdx])
        
        # Insert spraydrift information in TXW file
        lineIdx = get_lineIndex(txwContent,"table Loadings")
        for evt in (hydrologyMassLoadingsTable.loc[hydrologyMassLoadingsTable.LoaDrf>0]).iterrows():
           spraydriftStr = ' '.join([evt[1].Time.strftime('%d-%b-%Y-%Hh%M'), 'Drift', str(0.0), str(0.0), str(evt[1].LoaDrf*self.scaleFacDrift)])
        ...
        txwContent[lineIdx] = re.sub("<downstreamReach>", downstreamIdStr, txwContent[lineIdx])
        ...
        waterBodyVals = [reach.length, reach.nSegments, reach.width, reach.slope, DepWatDefPer]
        waterBodyStr = ''
        for i in range(len(varNameWaterBody)):
            varNameWaterBody[i] = varNameWaterBody[i].center(varWidthWaterBody[i])
            varUnitsWaterBody[i] = ('('+varUnitsWaterBody[i]+')').center(varWidthWaterBody[i])
            waterBodyStr = waterBodyStr + str(waterBodyVals[i]).center(varWidthWaterBody[i])
        ...
        txwContent[lineIdx-1] = txwContent[lineIdx-1].replace("<substanceName>",self.substanceNames[0])
        propSkip = ['NumDauWat','FraPrtDauWat','NumDauSed','FraPrtDauSed','SubName.1','SubName.2']
        substanceTable = pandas.read_csv(os.path.join(self.inputDir,self.toxswaConfig['substanceFile']),
                                         skiprows= [1], index_col = 0)
        propUnits = pandas.read_csv(os.path.join(self.inputDir,self.toxswaConfig['substanceFile']),nrows = 1)
        lineIdx = get_lineIndex(txwContent,"<substanceProperties>")
        del txwContent[lineIdx-1]
        if len(self.substanceNames) > 1:
            Raise(NotImplementedError("Metabolites not yet implemented, i.e. only one substance allowed"))
        for substance in self.substanceNames:
            for prop in list(substanceTable.columns):
                if prop in propSkip: continue
                line = '{:<15g}{}_{} ({})\n'.format(substanceTable.loc[substance,prop],prop,substance,propUnits[prop][0])
        ...
        txw_file = open(os.path.join(self.workDir,"Reach" + str(reach.ID) + "_tmp.txw"), "w")
    ...
    def write_hydFile(self,reach,hydrologyMassLoadingsTable):
        ...
        timeStep = [(time - timeStart).total_seconds()/secondsPerDay for i,time in hydrologyMassLoadingsTable.Time.iteritems()]
        ...
        outFile = open(os.path.join(self.workDir,'Reach' + str(reach.ID) + '.hyd'),'w')
    ...
    def update_txwFile(self,reach,timeStepWater,timeStepSediment):
        txw_file = open(os.path.join(self.workDir,"Reach" + str(reach.ID) + "_tmp.txw"), "rt")
        ...
        txw_file = open(os.path.join(self.workDir,"Reach" + str(reach.ID) + ".txw"), "wt")
        txw_file.write("".join(txwContent))
        txw_file.close()

    def hyd_exists(self,reach):
        return os.path.isfile(os.path.join(self.workDir,'Reach' + str(reach.ID) + '.hyd'))

    def txw_exists(self,reach):
        return os.path.isfile(os.path.join(self.workDir,"Reach" + str(reach.ID) + "_tmp.txw"))
    ...
    def mfu_exists(self,reach):
        return os.path.isfile(os.path.join(self.workDir,"Reach" + str(reach.ID) + ".mfu"))
    ...
    def run(self,reach):
        ...
        """
        report = {'reachID': reach.ID, 'status': Toxswa.flagOk, 'runTimeToxswa': None, 'runTimeTotal': None, 'timeStepSediment': None}
        ...
        try:
            while timeStepSediment >= self.toxswaConfig.getfloat('timeStepMin'):
                ...
                result = subprocess.run('"' + self.runToxswaCommand + '"' + " Reach" + str(reach.ID), 
                                        shell=True, cwd = self.workDir, stdout=subprocess.DEVNULL, 
                ...
                # check for errors
                if os.path.isfile(os.path.join(self.workDir,'Reach' + str(reach.ID) + '.ERR')):
        ...
        for subst in substNames:
            for outputVar in self.outputVars:
                if table is None:
                    ...
                    table[outputVar + '_' + subst] = self.get_output(reach,outputVar,subst)[outputVar + '_' + subst]
        if reach.skip and self.dummyOutTable is None: self.dummyOutTable = table
        return(table)
    ...
    def get_output(self,reach,varName,substName):
        ...
        """
        if reach.skip: return(self.get_output_dummy(reach,varName,substName))
        ...
        toxswa_output = pandas.read_csv(tmp,delim_whitespace=True,header=None, 
                                        names = ['timestep','date_time',varName + '_' + substName], 
                                        parse_dates=['date_time'],
                                        infer_datetime_format=True, usecols = [0,1,3])
        tmp.close()
        return toxswa_output

    def get_output_dummy(self,reach,varName,substName):
        dateTimes=pandas.date_range(self.startTime,self.endTime+datetime.timedelta(hours=1),freq='H')
    ...
    def write_dummy_mfu(self,reach):
        ...
        if not self.mfs_exists(downstrReachId): return
        ...
        file_mfuTempl = open(os.path.join(self.toxswaConfig['toxswaDir'],self.mfuTemplateFile),'r')
    ...
    def checkRun(self,reach):
        ...
        if os.path.isfile(logFileName):
            with open(logFileName,'rt') as logFile:
                ...
                if not re.search(regexPattern,logFileContents[-1]) is None: ret = True
        return(ret)
