...
def concGraph(catchment, timeStepRange, nPlot=100):
    ...
    timeSteps = np.linspace(
        idStart, idEnd, num=min(nPlot, idEnd - idStart), dtype=np.intc
    )
...
def concMap(
    ...
    for reach in catchment:
        if conc is None:
            conc = pandas.read_csv(
                os.path.join(outputDir, reach.ID + ".csv"),
                usecols=["timestep", "date_time", varName],
            )
            conc.columns = [reach.ID if x == varName else x for x in list(conc.columns)]
        else:
            conc[reach.ID] = pandas.read_csv(
                os.path.join(outputDir, reach.ID + ".csv"), usecols=[varName]
            )
    ...
    for idx, step in enumerate(timeSteps):
        ...
        catchment.catchmentMap(
            colorVals,
            valRange=[0, maxConc],
            fileName=os.path.join(outputDir, baseName + "_" + "{:03d}".format(idx))
            + format,
            title=title,
            linewidth=5,
        )
...
def processInputData(varName, config, catchment, dirName):
    ...
    endTime = dt.datetime.strptime(
        config["general"]["endDateSim"], "%d-%b-%Y"
    ) + dt.timedelta(hours=23)

    outputTable = pandas.read_csv(
        os.path.join(
            config["general"]["inputDir"], catchment[0].hydrologyMassLoadingsFile
        ),
        header=0,
        parse_dates=[0],
    ...
    filter = (outputTable.Time >= startTime) & (
        outputTable.Time <= endTime + dt.timedelta(hours=1)
    )
    ...
    for reach in catchment[1:]:
        var = pandas.read_csv(
            os.path.join(
                config["general"]["inputDir"], reach.hydrologyMassLoadingsFile
            ),
            header=0,
            skiprows=skipRows,
            usecols=[varName],
        ...
        if var == "LoaDrf":
            var = var * config.getfloat("toxswa", "scaleFacDrift")
        if any(var[varName] > 0):
            outputTable[reach.ID] = var

    outputTable.to_csv(
        os.path.join(dirName, varName + ".csv"), index=False, header=True
    )


def processCatchmentMassBalance(
    catchment, substanceName, outputDir, timeStepRange, fileName="catchment_massBalance"
):
    timeStepOut = 3600
    ...
    for reach in catchment:
        if massBal is None:
            massBal = pandas.read_csv(
                os.path.join(outputDir, reach.ID + ".csv"), usecols=fluxVars + stateVars
            )
            timeStep = pandas.read_csv(
                os.path.join(outputDir, reach.ID + ".csv"), usecols=["timestep"]
            )
            dateTime = pandas.read_csv(
                os.path.join(outputDir, reach.ID + ".csv"), usecols=["date_time"]
            )
        else:
            massBalReach = pandas.read_csv(
                os.path.join(outputDir, reach.ID + ".csv"), usecols=fluxVars + stateVars
            )
    ...
    for reach in catchment.ouletReaches:
        if outFlow is None:
            outFlow = pandas.read_csv(
                os.path.join(outputDir, reach.ID + ".csv"), usecols=[outFlowVarName]
            )
        else:
            outFlow = outFlow + pandas.read_csv(
                os.path.join(outputDir, reach.ID + ".csv"), usecols=[outFlowVarName]
            )
    ...
    massBal[masBalErrorVarName] = massBal[fluxVars].sum(axis=1) - massBal[
        stateVars
    ].sum(axis=1)
    ...
    for var in fluxVars:
        plot = plt.plot(
            timeStep["timestep"].iloc[idStart:idEnd],
            massBal[var].iloc[idStart:idEnd],
            label=var,
        )
    ...
    plt.plot(
        timeStep["timestep"].iloc[idStart:idEnd],
        massBal[masBalErrorVarName].iloc[idStart:idEnd],
    ...
    if not fileName is None:
        plt.savefig(os.path.join(outputDir, fileName + ".pdf"))


def plotConcHistograms(
    catchment,
    outputDir,
    substance,
    timePeriod,
    nBins=None,
    stepSize=1,
    fileName="catchment_histogram.pdf",
    percCutoff=None,
):
    varName = "ConLiqWatTgtAvg" + "_" + substance
    dirName = os.path.dirname(fileName)
    ...
    conc_sel = conc.loc[
        (conc.date_time >= timePeriod[0]) & (conc.date_time <= timePeriod[1]), :
    ]
...
def extractOutputVar(
    varName,
    substName,
    outputType=1,
    omitZeroReaches=False,
    cutoffQuantile=None,
    transpose=False,
    dirName=None,
    files=None,
    ...
    if files is None:  # no file list specified
        ...
        files = list(
            map(
                os.path.basename,
                glob.glob(os.path.join("" if dirName is None else dirName, "*.csv")),
            )
        )
        if "diagnostics.csv" in files:
            files.remove("diagnostics.csv")
    elif type(files) is str:  # files is string
        if "*" in files:  # wildcard
            files = list(
                map(
                    os.path.basename,
                    glob.glob(os.path.join("" if dirName is None else dirName, files)),
                )
            )
        else:  # one file
            files = [files]
    ...
    if not reachSel is None:
        ...
        for ID in reachSel:
            for idx, file in enumerate(files):
                if (not reachIDs is None and reachIDs[idx] == ID) or (
                    re.findall("[Rr]\d+", os.path.basename(file))[0] == ID
                ):
                    sel[idx] = True
    ...
    for file in pgb.progressbar(files):
        ...
        if outTable is None:
            outTable = pandas.read_csv(
                file,
                usecols=["timestep", "date_time", varSubstName],
                parse_dates=["date_time"],
                infer_datetime_format=True,
            )
            ...
            if not period is None:
                selIdx = (outTable["date_time"] >= period[0]) & (
                    outTable["date_time"] <= period[1]
                )
            ...
            if outputType == 1:
                reachTable = pandas.read_csv(
                    file,
                    usecols=[varSubstName],
                    squeeze=True,
                    skiprows=skipIdx,
                    index_col=False,
                )
                ...
                reachTable = pandas.read_csv(
                    file, usecols=[varSubstName], skiprows=skipIdx, index_col=False
                )
    ...
    if outputType == 2:
        outTable.columns = list(outTable.columns[:3]) + ["Value"]
    ...
    if transpose:
        outTable.T.to_csv(
            outputFile, index=True, header=False, date_format=dateTimeFormat
        )
    else:
        outTable.to_csv(
            outputFile, index=False, header=True, date_format=dateTimeFormat
        )
